...
public class OrderFacade {
    ...
    public OrderResponse createOrder(Long userId, OrderCreateRequest request) {
        ...
        for (OrderCreateItemRequest itemRequest : request.getItems()) {
            ...
            EventProductResponse eventProduct = commerceServiceClient.getEventProduct(itemRequest.getEventProductId());
            ProductResponse product = commerceServiceClient.getProduct(eventProduct.productId());

            queueStockServiceClient.validatePurchaseAccess(eventProduct.id(), userId);

            queueStockServiceClient.reserveStock(new StockRequest(eventProduct.id(), itemRequest.getQuantity()));
            ...
            orderedEventProductIds.add(eventProduct.id());
        }
        ...
        for (Long eventProductId : orderedEventProductIds) {
            queueStockServiceClient.completeQueue(eventProductId);
        }
    ...
    public void cancelOrder(Long userId, Long orderId) {
        ...
        for (OrderItemEntity item : order.getOrderItems()) {
            queueStockServiceClient.releaseStock(
                    new StockRequest(item.getEventProductId(), item.getQuantity())
            );
        }
